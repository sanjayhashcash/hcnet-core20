...
class QuorumIntersectionChecker
{
  public:
    ...
    create(QuorumTracker::QuorumMap const& qmap,
           ...
           hcnet_default_random_engine::result_type seed, bool quiet = false);
    ...
    static std::set<std::set<NodeID>> getIntersectionCriticalGroups(
        ...
        std::optional<hcnet::Config> const& cfg,
        std::atomic<bool>& interruptFlag,
        hcnet_default_random_engine::result_type seed);

    static std::set<std::set<NodeID>> getIntersectionCriticalGroups(
        QuorumSetMap const& qmap, std::optional<hcnet::Config> const& cfg,
        std::atomic<bool>& interruptFlag,
        hcnet_default_random_engine::result_type seed);
