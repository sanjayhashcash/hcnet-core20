...
struct XDRFieldResolver
{
    ...
    operator()(T const& k, char const* fieldName)
    {
        if (checkLeafField(fieldName))
        {
            mResult = hcnet::KeyUtils::toStrKey(k);
        }
    }
    ...
    operator()(T const& k, char const* fieldName)
    {
        if (checkLeafField(fieldName))
        {
            ...
            case SC_ADDRESS_TYPE_CONTRACT:
                mResult = hcnet::strKey::toStrKey(
                              hcnet::strKey::STRKEY_CONTRACT, k.contractId())
                              .value;
                break;
            case SC_ADDRESS_TYPE_ACCOUNT:
                mResult = hcnet::KeyUtils::toStrKey(k.accountId());
                break;
            default:
                mResult = "UNKNOWN";
                break;
    ...
    operator()(T const& asset, char const* fieldName)
    {
        ...
        case ASSET_TYPE_CREDIT_ALPHANUM12:
        {
            ...
            (*this)(hcnet::getIssuer(asset), "issuer");
            break;
        }
        default:
            mResult = "UNKNOWN";
