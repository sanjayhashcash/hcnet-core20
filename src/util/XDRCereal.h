...
void cereal_override(cereal::JSONOutputArchive& ar,
                     const hcnet::SCAddress& addr, const char* field);

void cereal_override(cereal::JSONOutputArchive& ar,
                     const hcnet::ConfigUpgradeSetKey& key,
                     const char* field);
...
void cerealPoolAsset(cereal::JSONOutputArchive& ar, const hcnet::Asset& asset,
                     const char* field);
...
template <typename T>
typename std::enable_if<std::is_same<hcnet::Asset, T>::value ||
                        std::is_same<hcnet::TrustLineAsset, T>::value ||
                        std::is_same<hcnet::ChangeTrustAsset, T>::value>::type
                ...
                const char* field)
{
    ...
    case hcnet::ASSET_TYPE_CREDIT_ALPHANUM12:
    {
        ...
        std::string code;
        if (asset.type() == hcnet::ASSET_TYPE_CREDIT_ALPHANUM4)
        {
            hcnet::assetCodeToStr(asset.alphaNum4().assetCode, code);
        }
        else
        {
            hcnet::assetCodeToStr(asset.alphaNum12().assetCode, code);
        }

        xdr::archive(ar, code, "assetCode");
        xdr::archive(ar, hcnet::getIssuer(asset), "issuer");
        ar.finishNode();
        break;
    }
    default:
