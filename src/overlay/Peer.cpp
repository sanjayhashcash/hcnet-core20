...
Peer::sendSendMore(uint32_t numMessages)
{
    ...
    auto m = std::make_shared<HcnetMessage>();
    m->type(SEND_MORE);
...
void
Peer::sendAuthenticatedMessage(std::shared_ptr<HcnetMessage const> msg)
{
...
Peer::recvRawMessage(HcnetMessage const& hcnetMsg)
{
    ...
    releaseAssert(isAuthenticated() || hcnetMsg.type() == HELLO ||
                  hcnetMsg.type() == AUTH || hcnetMsg.type() == ERROR_MSG);
    mAppConnector.getOverlayManager().recordMessageMetric(hcnetMsg,
                                                          shared_from_this());
...
Peer::recvGetTxSet(HcnetMessage const& msg)
{
    ...
    {
        auto newMsg = std::make_shared<HcnetMessage>();
...
Peer::recvAuth(HcnetMessage const& msg)
{
    ...
    auto sendCb = [weakSelf](std::shared_ptr<HcnetMessage const> msg) {
    ...
    mTxAdverts->start([weakSelf](std::shared_ptr<HcnetMessage const> msg) {
        auto self = weakSelf.lock();
        if (self)
        {
