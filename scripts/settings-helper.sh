...
SECRET_KEY="$1"
PUBLIC_KEY=$(./hcnet-core convert-id "$SECRET_KEY" | sed -n '4p' | awk '{print $NF}')
echo "PUBLIC_KEY is $PUBLIC_KEY"

PUBNET_HORIZON="https://horizon.hcnet.org/accounts"
PUBNET_PASSPHRASE="Public Global Hcnet Network ; September 2015"

TESTNET_HORIZON="https://horizon-testnet.hcnet.org/accounts"
...
OUTPUT="$(echo $SECRET_KEY | ./hcnet-core get-settings-upgrade-txs "$PUBLIC_KEY" "$SEQ_NUM" "$PASSPHRASE" --xdr $(hcnet-xdr encode --type ConfigUpgradeSet $3) --signtxs)"
